...
const AdminSidebar = ({ activeTab, setActiveTab, onLogout, isCollapsed, setIsCollapsed }) => {
    ...
    return (
        ...
        >
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    {!isCollapsed && (
                        <div className="flex flex-col">
                            ...
                            <p className="text-[10px] text-slate-200 dark:text-primary-light/60 font-medium mt-1">Control Center</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            >
                {menuGroups.map((group, groupIdx) => (
                    <div key={groupIdx} className="space-y-1">
                        {!isCollapsed && (
                            <p className="px-4 text-[10px] font-bold text-slate-400 dark:text-primary-light/40 uppercase tracking-widest mb-3">
                                {group.title}
                            </p>
                        )}
            ...
            {/* Sidebar Bottom - Professional Footer */}
            <div className={`p-2 sm:p-3 bg-primary dark:bg-slate-950 border-t border-white/5 mt-auto transition-all transition-colors`}>
                <button
                    onClick={onLogout}
